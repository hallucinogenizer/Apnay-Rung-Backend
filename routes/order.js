    ...
    */
    ...
    if (valid_input !== true) {
        ...
        if (req.userObject.typeOfUser == "customer") {
            ...
            let values = ['NOW()', req.userObject.id, req.body.delivery_status, JSON.stringify([]), req.body.totalamount, false, req.body.items, req.body.name, req.body.email, req.body.phone, req.body.billing_address, req.body.shipping_address, req.body.payment_method]
    ...
    try {
        ...
        for (order in result.rows) {
            ...
            for (item in result.rows[order].review) {
                if (result.rows[order].review[item][0] == req.params.item_id) {
                    exists = true
                    index = item
                }
            }
